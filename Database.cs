namespace LibraryRecordSystem
{
    class Database
    {
        ...
        public bool VerifyCustomerID(int customerID)
        {
            foreach (Customer registeredCustomer in Customers)
            {
                if (registeredCustomer.CustomerID == customerID)
                {
                    return true;
                }
            }
        ...
        public void RemoveBook(int isbnNumb)
        {
            ...
            if (bookToRemove != null)
            {
                ...
                Console.WriteLine($"{bookToRemove.NameOfBook} with ISBN {isbnNumb} removed from the database.");
            }
            else
            {
                Console.WriteLine($"{bookToRemove.NameOfBook} with ISBN {isbnNumb} not found in the database.");
            }
        }
        ...
        public void AddCustomer(string customerName, DateTime dateOfBirth)
        {
            ...
            Console.WriteLine($"{customerName} with date of birth {dateOfBirth.ToShortDateString()} added to the database with customer ID: {customerID}.");
        }
        ...
        public void AddLoan(int customerID, int isbnNumb)
        {
            ...
            Loans.Add(loan);
            Console.WriteLine($"{customerID} has been given the book on {DateTime.Now.AddDays(14).ToShortDateString()}.");
        }

        public void ReturnLoan(int customerID, int isbnNumb)
        {
        ...
        public void SearchForBook(string bookName)
        {
            foreach (Book registeredBook in Books)
            {
                if (registeredBook.NameOfBook == bookName)
                {
                    Console.WriteLine(registeredBook.ToString());
                }
                else
        ...
        public void SearchForLoansToCustomer(int customerID)
        {
            foreach (Loan registeredLoan in Loans)
            {
